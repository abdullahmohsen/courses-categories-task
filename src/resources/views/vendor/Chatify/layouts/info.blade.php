...
<div class="avatar av-l">
        ...
        @if (tenant('id') == null)
            <img alt="image"
                src="{{ Auth::user()->avatar ? asset(Auth::user()->avatar) : asset('assets/img/avatar/avatar-1.png') }}"
                class="rounded-circle mr-1">
        @else
            @if (config('filesystems.default') == 'local')

                <img id="avatar-img" class="rounded-circle mr-1"
                    src="{{ Auth::user()->avatar ? asset(tenant('id') . '/' . Auth::user()->avatar) : asset('assets/img/avatar/avatar-1.png') }}"
